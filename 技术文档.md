...
| `round_done` | `── Round N 完成 (已完成/总模型数) ──` |
...
AGORA_MODEL_SOURCE, AGORA_ALLOWED_MODELS, AGORA_DEFAULT_COUNCIL_MODELS, AGORA_SECRETARY_MODEL
```
...
export const RECOMMENDED_FREE_MODEL_WHITELIST = ['openai/gpt-oss-120b:free', 'qwen/qwen3-next-80b-a3b-instruct:free', 'meta-llama/llama-3.3-70b-instruct:free', 'nousresearch/hermes-3-llama-3.1-405b:free', 'google/gemma-3-27b-it:free'];
export const RECOMMENDED_FREE_COUNCIL_MODELS = ['openai/gpt-oss-120b:free', 'qwen/qwen3-next-80b-a3b-instruct:free', 'meta-llama/llama-3.3-70b-instruct:free'];
export const DEFAULT_SECRETARY_MODEL = 'openai/gpt-oss-120b:free';
...
- `AGORA_MODEL_SOURCE` 当前只允许 `openrouter`
- `AGORA_ALLOWED_MODELS` 定义当前环境允许使用的模型白名单
- `AGORA_DEFAULT_COUNCIL_MODELS` 定义当前环境默认参与 council 的模型列表
- `AGORA_SECRETARY_MODEL` 定义当前环境默认 secretary 模型；Phase A1 若未单独接线，可先复用 `AGORA_DEFAULT_COUNCIL_MODELS[0]`
- `test` 环境默认采用 5 个免费模型白名单，其中默认 council 参与模型为 3 个
- `prod` 环境允许使用不同白名单，但必须通过同一组环境变量注入，不得写死在 CLI 或 Web
...
| ID | 流程 | 验证 |
|----|------|------|
| E01 | 完整 test 默认 3 模型 3 轮 + summary | 全流程跑通 + 积分正确 |
...
| G03 | `agora council run` 按环境默认模型完整跑通（test 默认 3 模型 3 轮） | 手动运行 3 场以上 |
...
**目标**：用最少代码跑通"单模型问答 + 环境默认模型议会讨论"核心链路。

**范围**：DB schema + migrations + seed data（11 张表）、OpenRouter 适配层、安全层（输入校验基础版）、Orchestrator 核心（状态机 + 3 轮共识）、StreamHub 容错（timeout/retry/skip/degraded）、Anonymizer、Secretary（zod 校验 + 降级）、ContextManager（结构化压缩）、session-starter（统一启动路径）、CLI 骨架 + `agora ask` + `agora council run`、JSONL 事件日志、基于环境变量的模型白名单与默认模型选择。

**交付物**：`agora ask` 跑通 + `agora council run` 按环境默认模型完整 3 轮跑通（test 默认 3 个参与模型）+ 事件流写入 JSONL + DB 持久化正确。
...
| Task-004 | OpenRouter 适配层 | Task-001 | 能调用单个模型获得流式响应 + 常量全部导出 + 环境化模型白名单配置可读 |
| Task-005 | 安全层 | Task-002 | 注入 pattern 拦截 + topic_hash 去重 + 输入长度上限 + Plan 日限 + normalizeTopic()。**U18, U19, U20** |
| Task-008 | Orchestrator 核心 | Task-004, Task-005 | 状态迁移白名单 + CAS + 执行锁 + 按环境默认模型 3 轮跑通（test 默认 3 模型）+ canContinue()。**U01, U02, U04** |
...
- [ ] 召集议会可用（按环境默认模型完成 3 轮共识模式跑通，test 默认 3 模型）
- [ ] 匿名互评生效（第 2 轮无模型名）
